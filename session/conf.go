...
type TCPConf struct {
	...
	// specifies "t₀" in the range of (1, 255) seconds with a default of 30.
	...
	// The standard specifies "k" in the range of (1, 32767) with a default
	// of 12. See IEC 60870-5-104, subclause 5.5.
	SendUnackMax uint
	...
	// the range of (1, 255) seconds with a default of 15. See IEC
	// 60870-5-104, figure 18.
	...
	// exceed two thirds of SendUnackMax. The standard specifies "w" in the
	// range of (1, 32767) with a default of 8. See IEC 60870-5-104,
	// subclause 5.5.
	RecvUnackMax uint
	...
	// second. The standard specifies "t₂" in the range of (1, 255) seconds
	// with a default of 10. See IEC 60870-5-104, figure 10.
	RecvUnackTimeout time.Duration

	// Amount of idle time needed to trigger "TESTFR" keep-alives. The
	// standard recommends "t₃" in the range of (1 second, 48 hours) and the
	// default is set to 20 seconds. See IEC 60870-5-104, subclause 5.2.
...
func (c *TCPConf) check() *TCPConf {
	if c.ConnectTimeout == 0 {
		...
		panic(`ConnectTimeout "t₀" not in range (1, 255)s`)
	}

	if c.SendUnackMax == 0 {
		c.SendUnackMax = 12
	} else if c.SendUnackMax < 1 || c.SendUnackMax > 32767 {
		panic(`SendUnackMax "k" not in the range (1, 32767)`)
	}
	...
	if c.SendUnackTimeout == 0 {
		...
		panic(`SendUnackTimeout "t₁" not in range (1, 255)s`)
	}

	if c.RecvUnackMax == 0 {
		c.RecvUnackMax = 8
	} else if c.RecvUnackMax < 1 || c.RecvUnackMax > 32767 {
		panic(`RecvUnackMax "w" not in the range (1, 32767)`)
	}
	...
	if c.RecvUnackTimeout == 0 {
		...
		panic(`RecvUnackTimeout "t₂" not in range (1, 255)s`)
	}
